...
#pragma comment(lib, "D:\\Programms\\Microsoft Visual Studio 2022\\VC\\Tools\\MSVC\\14.34.31933\\lib\\x86\\Glaux.lib")
#pragma comment(lib, "legacy_stdio_definitions.lib")

/*Структура для хранения заголовка файла изображения*/
struct Zagolovok
{
	int16_t shirina; /*Ширина*/
	int16_t vysota; /*Высота*/
	GLenum formatCveta; /*Формат представления цвета*/
	GLenum formatKomponenty; /*Формат данных компоненты цвета*/
	int16_t kol_voKomponent; /*Количество компонент цвета*/
};
...
Izobrajenie* ChtenieIzobrajeniyaIzFaila(const char* szFileName)
{
	...
	Izobrajenie* iz = (Izobrajenie*)malloc(sizeof(Izobrajenie)); //Создание структуры дляхранения изображения
	// Открытие файла
	pFile = fopen(szFileName, "rb");
	if (pFile == NULL) return NULL;
	// Считываем заголовок
	fread(&(iz->zagolovok.shirina), sizeof(int16_t), 1, pFile);
	fread(&(iz->zagolovok.vysota), sizeof(int16_t), 1, pFile);
	fread(&(iz->zagolovok.formatCveta), sizeof(GLenum), 1, pFile);
	fread(&(iz->zagolovok.formatKomponenty), sizeof(GLenum), 1, pFile);
	fread(&(iz->zagolovok.kol_voKomponent), sizeof(int16_t), 1, pFile);
...
GLint SohranenieIzobrajeniyaVfail(const char* szFileName)
{
	...
	iz->zagolovok.formatCveta = GL_BLUE;
	iz->zagolovok.formatKomponenty = GL_SHORT;
	iz->zagolovok.kol_voKomponent = 3; // Изменение количества компонент на 3
	...
	// Запись заголовка
	fwrite(&(iz->zagolovok.shirina), sizeof(int16_t), 1, pFile);
	fwrite(&(iz->zagolovok.vysota), sizeof(int16_t), 1, pFile);
	fwrite(&(iz->zagolovok.formatCveta), sizeof(GLenum), 1, pFile);
	fwrite(&(iz->zagolovok.formatKomponenty), sizeof(GLenum), 1, pFile);
	fwrite(&(iz->zagolovok.kol_voKomponent), sizeof(int16_t), 1, pFile);
	...
	// Работа с файлом завершается
	fclose(pFile);
	// Возвращает 0 в случае успешного завершения функции
	return 0;
}
...
void Pererisovka(void)
{
	...
	case 1: /*Загрузка изображения из файла *.bmp*/
		pImage = auxDIBImageLoadA("C:\\Users\\Ильдар\\Desktop\\Учеба\\ComputerGraphic\\Labs\\Лаба 6\\texture_2.bmp");
		...
		izobr_bmp->zagolovok.formatCveta = GL_BLUE;
